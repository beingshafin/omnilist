...
let SCRIPT = path.join(__dirname, 'omnilist.js');
if (!fs.existsSync(SCRIPT)) {
  const candidates = [
    path.join(PROJECT_ROOT, 'src', 'omnilist.js'),
    path.join(PROJECT_ROOT, 'omnilist.js'),
    path.join(__dirname, '..', 'src', 'omnilist.js'),
  ];
...
// entirely; otherwise config/default.jsonc, config/config.jsonc, config/config.local.jsonc.
function configFilePaths() {
  const defPath = path.join(PROJECT_ROOT, 'config', 'default.jsonc');
  ...
  const local = fs.existsSync(l1) ? l1 : fs.existsSync(l2) ? l2 : l1;
...
function resolveProvidersFile(cfg) {
  return resolveExisting(cfg && cfg.paths && cfg.paths.providers_csv, 'providers.csv');
}
...
function readProviders() {
  const cfg = loadConfig();
  const file = resolveProvidersFile(cfg);
...
function handleApi(req, res, url) {
  ...
  if (req.method === 'GET' && route === '/api/config') {
    const { defaults: defPath, primary, local } = configFilePaths();
    const currentDefaults = typeof loadDefaults === 'function' ? loadDefaults() : DEFAULTS;
  ...
  if (req.method === 'POST' && route === '/api/config/push-default') {
    return readBody(req).then((body) => {
      const { section, keys, effective } = body;
      if (!keys || !Array.isArray(keys) || !effective) {
        return sendJson(res, 400, { error: 'expected { section, keys, effective }' });
      }
      const { defaults: defPath, primary, local } = configFilePaths();
      const currentDefParsed = parseJsoncFile(defPath);
      const defData = currentDefParsed.ok && currentDefParsed.data ? currentDefParsed.data : deepMerge(JSON.parse(JSON.stringify(DEFAULTS)), {});

      // For each key path in keys, copy the value from effective into defData
      for (const pathArr of keys) {
        let srcNode = effective;
        ...
        let targetNode = defData;
        for (let i = 0; i < pathArr.length - 1; i++) {
          const p = pathArr[i];
          if (!targetNode[p] || typeof targetNode[p] !== 'object' || Array.isArray(targetNode[p])) {
            targetNode[p] = {};
          }
          targetNode = targetNode[p];
        }
        targetNode[pathArr[pathArr.length - 1]] = JSON.parse(JSON.stringify(srcNode));
      }

      // Backup default.jsonc if exists
      if (fs.existsSync(defPath)) fs.copyFileSync(defPath, defPath + '.bak');

      // Preserve default header if present
      const prev = fs.existsSync(defPath) ? fs.readFileSync(defPath, 'utf8') : '';
      const keep = preservedHeader(prev);
      const header = keep !== null && keep !== undefined ? keep : `/*\n  OMNILIST DEFAULT CONFIGURATION (default.jsonc)\n*/\n\n`;
      fs.writeFileSync(defPath, header + toJsoncBody(defData), 'utf8');

      // If local override exists, re-diff and remove keys that now match defaults
      if (local && fs.existsSync(local)) {
        fs.copyFileSync(local, local + '.bak');
        const base = parseJsoncFile(primary);
        const baseMerged = deepMerge(JSON.parse(JSON.stringify(defData)), base.ok && base.data ? base.data : {});
        const diff = diffConfig(effective, baseMerged);
        fs.writeFileSync(local, LOCAL_HEADER + toJsoncBody(diff), 'utf8');
      }
  ...
  if (req.method === 'POST' && route === '/api/config') {
    return readBody(req).then((body) => {
      ...
      if (target.endsWith('config.local.jsonc')) {
        const { defaults: defPath, primary } = configFilePaths();
        const defParsed = parseJsoncFile(defPath);
        const currentDef = defParsed.ok && defParsed.data ? defParsed.data : DEFAULTS;
        const base = parseJsoncFile(primary);
        const baseMerged = deepMerge(JSON.parse(JSON.stringify(currentDef)), base.ok && base.data ? base.data : {});
  ...
  if (req.method === 'GET' && route === '/api/meta') {
    ...
    return sendJson(res, 200, {
      ...
      scriptDir: PROJECT_ROOT.replace(/\\/g, '/'),
...
function createServer() {
  return http.createServer((req, res) => {
    ...
    const candidates = [
      ...
      path.join(PROJECT_ROOT, 'src', 'dashboard.html'),
    ...
    if (!htmlFile) {
      ...
      return res.end('dashboard.html not found: ' + candidates.join(', '));
    }
    try {
      ...
      res.writeHead(500, { 'Content-Type': 'text/plain' });
      res.end('Error reading dashboard.html: ' + e.message);
    }
  });
}
...
function start(opts) {
  ...
  const tryListen = (p) => {
    ...
    server.listen(p, '127.0.0.1', () => {
      ...
      currentPort = p;
      try { fs.writeFileSync(path.join(PROJECT_ROOT, '.gui.port'), String(p), 'utf8'); } catch (e) {}
      try { fs.writeFileSync(path.join(PROJECT_ROOT, '.gui.pid'), String(process.pid), 'utf8'); } catch (e) {}
      console.log('');
