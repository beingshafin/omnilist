...
Paths may be absolute, relative (to the script dir), or use `~` for your home directory. The environment variables `MODELS_TEST`, `JSONC_TEST`, `KILO_TEST`, `T3_DATA_DIR`, `DSH_SETTINGS_FILE`, `DSH_CREDENTIALS_FILE`, `PI_FILE`, `ZCODE_FILE`, `OPENCODEX_FILE` / `OCX_FILE`, and `OMNILIST_CONFIG` still override paths / the config location.
...
omnilist ocx             # Sync Router models into opencodex (config.json) — custom_* provider (alias: opencodex)
...
Adapters are uniform: every harness gets `{ router_adapters: [...], rest_adapters: [...] }` with free-form strings (written verbatim into that harness's field): **opencode/kilo → `npm`**, **dsh → `api`**, **pi → `api`**, **zcode → `kind`**, **opencodex → `adapter`**. DSH legacy `router_apis`/`rest_apis` are still accepted. Single adapter → bare key `custom_<provider>` (and `_2`, `_3` for duplicate provider rows, first stays bare); multi-adapter → `__<adapter>` suffix on every key (verbatim, e.g. `custom_omniroute__@ai-sdk/anthropic`) combined as `custom_<provider>[_<N>]__<adapter>` (instance outer, adapter inner: `custom_agentrouter__a`, `custom_agentrouter_2__a`, `custom_agentrouter__b`, ...). T3 has no adapters / harnesses.
...
opencodex stores its catalog at `~/.opencodex/config.json` (configurable via `paths.opencodex_file` and env `OPENCODEX_FILE` / `OCX_FILE`). Enable with `targets.opencodex_router` / `targets.opencodex_rest`. Keys are `custom_<provider>[_<N>]` (mirrors the Router/REST pattern, but with opencodex's own `adapter` / `apiKeyPool` shape):

- Router: single `providers["custom_<router>"]` entry (`adapter:"openai-chat"`, `baseUrl`, `allowPrivateNetwork:true`, `authMode:"key"`, `apiKey`, `apiKeyPool:[{id:8hex, key}]`, `liveModels:false`) and `defaultProvider="custom_<router>"`.
- REST: grouped by `provider` (every non-Router `providers.csv` row); models filtered to `provider + '/'` then stripped (`agentrouter/flash` → `flash`); duplicate rows for the same provider become `custom_<provider>`, `custom_<provider>_2`, … Each gets its own `customModels` entries with stripped `modelId`.

Router `customModels` keep the full `provider/model` id (`agentrouter/claude-opus-5`); REST `customModels` keep the stripped bare name (`claude-opus-5`). Each `customModels` entry is `{ id: uuid, provider: "custom_<provider>[_<N>]", modelId, addedAt }`. Unknown top-level keys (`port`, `openai`, `subagentModels`, `claudeCode.desktopProfile`, …) are preserved; `claudeCode` is ensured to `{ authMode:"proxy" }` (created if missing, otherwise only `authMode` forced to `proxy`). Preview: `models-ocx.csv`. Cleanup prunes `custom_*` providers and scoped `customModels` by flag/provider existence and `apiKey`/`baseUrl` renumbering; `openai` and non-`custom_*` keys are preserved. `defaultProvider` is only set by the Router sync; REST does not overwrite it.
